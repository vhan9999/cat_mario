...
void CGameStateRun::EventCtrl() {
	...
	if (player.coll.GetLeft() > 815 && !event_list["up_seal"]) {
			Enemy up_seal_1(880, 600, { {"resources/image/enemy/seal.bmp"} }); up_seal_1.steel = false; up_seal_1.able_touch = false; up_seal_1.speed_y = -19; up_seal_1.step_enemy_player_dead = true; enemys_arr.push_back(up_seal_1);
			event_list["up_seal"] = true;
		}
	}
		...
		if (!event_list["jump_back"] && event_list["jump_big_cat"] && player.coll.GetLeft() > 0 &&player.coll.GetLeft() < 300 && player.distance_count <= 3000) {
		...
		if (player.distance_count >= 4840) {
			...
			if ((player.map3frame > 85 && !event_list["boolean_question"]) || (player.map3frame > 12 && event_list["boolean_question"])) {
				for (auto &i : bricks_arr) {
					if (i.item == "orange_ball") {
						int randx = rand() % (3 - (-3) + 1) + (-3);
						int randy = rand() % (-18 - (-28) + 1) + (-28);
		...
		if (player.distance_count >= 3172 && player.distance_count < 3272 && !event_list["phase6-9"]) {
			int mountain_height = 132;
			int grass_height = 50;
			int pipeline_big_height = 240;
			int pipeline_mid_height = 180;
			...
			int ground = 776;
			/* phase 6 */
			int startBlock = 4440 - player.distance_count;
