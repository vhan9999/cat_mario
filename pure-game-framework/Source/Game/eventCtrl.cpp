...
void CGameStateRun::EventCtrl() {
	if (current_map == 1) { 
		// phase2
		if (player.enemy_shift_amount >= 600 && player.enemy_shift_amount <= 700 && !event_list["pipe_normal"]) { // event_list => add of event map
		...
		if (player.enemy_shift_amount >= 650 && player.enemy_shift_amount <= 750 && !event_list["up_seal_1"]) {
		...
		// phase 4
		if (player.enemy_shift_amount >= 2000 && player.enemy_shift_amount <= 2100 && !event_list["two_normal_1"]) {
		...
		// phase 5
		if (player.enemy_shift_amount >= 2780 && player.enemy_shift_amount <= 2880 && !event_list["down_seal_1"]) {
		...
		if (player.enemy_shift_amount >= 2550 && player.enemy_shift_amount <= 2700 && !event_list["three_fall_bricksss"]) {
			//three fall
		...
		// phase 8
		if (player.enemy_shift_amount >= 5122 && player.enemy_shift_amount <= 5222 && !event_list["two_normal_2"]) {
		...
		// phase 10
		if (player.enemy_shift_amount >= 6284 && player.enemy_shift_amount <= 6384 && !event_list["fall_normals"]) {
		...
		if (player.enemy_shift_amount >= 6555 && player.enemy_shift_amount <= 6655 && player.coll.GetTop() <= 200 && !event_list["yellow"]) {
